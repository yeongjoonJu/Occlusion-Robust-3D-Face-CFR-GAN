...
yaws_list = np.load(osp.join(d, 'AFLW2000-3D.pose.npy'))

# [1306, 462, 232], same as paper
yaws_list = np.load(osp.join(d, 'AFLW2000-3D-new.pose.npy'))

# origin
pts68_all_ori = np.load(osp.join(d, 'AFLW2000-3D.pts68.npy'))

# reannonated
pts68_all_re = np.load(osp.join(d, 'AFLW2000-3D-Reannotated.pts68.npy'))
roi_boxs = np.load(osp.join(d, 'AFLW2000-3D_crop.roi_box.npy'))
